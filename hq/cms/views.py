...
class AssessmentViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        if user["role"] == "MEMBER":
            ...
            ) | Assessment.objects.filter(assigned_to=user["user_id"])
        else:
...
class QuestionViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        assessment_queryset = Assessment.objects.filter(
            Q(creator=user["user_id"]) | Q(assigned_to=user["user_id"])
        )
        assessments = []
        ...
        if user["role"] != "ADMIN":
            queryset = queryset.filter(
                Q(creator=user["user_id"]) | Q(assessmentid__in=assessments)
            )
    ...
    def retrieve(self, request, pk=None):
        ...
        for question in queryset:
            ...
            if (
                question.creator.id == user["user_id"]
                or len(set(qassessments) & set(assessments)) != 0
                or user["role"] == "ADMIN"
            ):
                flag = True
            else:
        ...
        if flag:
            ...
            return Response(
                {"Not Authorized": "You don't have access to this entry"},
...
class CwfKtStage(APIView):
    ...
    def post(self, request):
        ...
        if not valid_data["role"].get("value"):
            ...
            Role.objects.filter(id=valid_data["role"].get("value")).update(
                name=valid_data["role"].get("label")
            )
        ...
        for i in range(len(valid_data.get("stage"))):
            ...
            if not cstage.get("value"):
                ...
                stage = Stage.objects.get(id=cstage.get("value"))
        ...
        for i in range(len(valid_data.get("wf"))):
            ...
            if not wfdata.get("value"):
                ...
                cwfun = Cwf.objects.get(id=wfdata.get("value"))
            ...
            for j in range(len(wfdata["keyTask"])):
                ...
                if not ktdata.get("value"):
                    ...
                    kt = Kt.objects.get(id=ktdata.get("value"))
    ...
    def get(self, request, format=None):
        ...
        for role in roleid:
            ...
            for cwf in cwfs:
                ...
                cwfcode = cwf.id
                kts = (
                    Kt.objects.filter(isdeleted=False).filter(cwf__id=cwf.id).distinct()
                )
                # print(kts)
                kt_array = []
                for kt in kts:
            ...
            stages = (
                Stage.objects.filter(isdeleted=False).filter(role__id=role).distinct()
            )
            stage_array = []
            for stage in stages:
                # print(stage.code)
        ...
        if "assessmentId" in data:
            ...
            for a in Assessment.objects.filter(id=aid):
                if a.status == "READY":
                    ...
                    return Response(
                        {"failure": "Status not apropriate for the transaction"},
    ...
    def post(self, request):
        ...
        if user.access_role == "ADMIN":
            ...
            if out == 0:
                ...
                return Response(
                    {"Error": out}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
        else:
            return Response(
                {"Not Authorized": "You don't have access to this entry"},
...
class AssessInfo(APIView):
    ...
    def get(self, request):
        ...
        assessment_obj = (
            Assessment.objects.filter(isdeleted=False).filter(code=code).distinct()
        )
        ...
        if assessment_obj:
            ...
            question_obj = (
                Question.objects.filter(isdeleted=False).filter(id__in=qlist).distinct()
            )
